...
def get_life_stats(address):
    ...
    for j in life_data.keys():
        ...
        qof.append(
            {
                "x": j,
                "y": int(sum(np.array(life) >= 60)) / len(life),
                "agents": [id[idx] for idx in range(len(life)) if life[idx] >= 60],
            }
        )
...
def get_parents(path, filename, ancestor_list, level=0):
    ...
    # Otherwise recursively find parent at each generation
        ...
        if type(parents[0]) == list:
            ...
            get_parents(
                path, str(tob_1) + "-" + str(id_1) + ".npy", ancestor_list, level
            )
            ...
            get_parents(
                path, str(tob_2) + "-" + str(id_2) + ".npy", ancestor_list, level
            )
                    ...
                    successor_list.append(
                        {
                            "child_of": filename,
                            "filename": os.path.join(
                                path, str(time) + "-" + str(offspring_id) + ".npy"
                            ),
                            "level": int(level),
                        }
                    )
